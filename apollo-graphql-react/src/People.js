...
const FetchFailed = () => (
  <p className="bg-red-600 bg-opacity-75 text-6xl text-color-white">
    Fetch failed
  </p>
)
...
const People = () => {
  const {
    ...
    data,
    client,
  } = useQuery(FIND_ALL_RECORDS_FROM_LOCAL_STORE, { pollInterval: 1500 })
  window.client = client
  const addPersonCompleted = () => {
    const dataFromCache = client.readQuery({query : FIND_ALL_RECORDS_FROM_LOCAL_STORE})
